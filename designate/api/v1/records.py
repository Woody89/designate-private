...
def create_record(domain_id):
    ...
    recordset_id = values['recordset_id']
    central_api = central_rpcapi.CentralAPI.get_instance()
    recordset_db = central_api.get_recordset(context, domain_id, recordset_id)
    values['name'] = recordset_db.name
    values['type'] = recordset_db.type
    values['ttl'] = recordset_db.ttl
    values.pop('recordset_id')
    ...
    record = _format_record_v1(record, recordset_db)
