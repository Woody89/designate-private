...
class ZonesController(rest.RestController):
    ...
    def post_all(self):
        ...
        if "email" not in zone.keys():
            # HF design, set zone default email value to example@zonename
            zone["email"] = "example@" + zone["name"][:-1]
    ...
    def delete_one(self, zone_id):
        ...
        # response = DesignateAdapter.render('API_v2', zone, request=request)
        return response
