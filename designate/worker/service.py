...
from designate.worker.tasks import zone as acltasks
...
class Service(service.RPCService, service.Service):
    ...
    def _do_acl_action(self, context, acl, action):
        pool = self.get_pool(acl.pool_id)
        tasks = acltasks.AclAction(
            self.executor, context, pool, acl, action)
    ...
    def _do_recordset_action(self, context, zone, recordset):
        pool = self.get_pool(zone.pool_id)
        task = zonetasks.RecordSetAction(
            self.executor, context, pool, zone, recordset, recordset.action
        )
        return self.executor.run(task)

    def _do_record_action(self, context, zone, record, recordset):
        pool = self.get_pool(zone.pool_id)
        task = zonetasks.RecordAction(
            self.executor, context, pool, zone, record, record.action, recordset
        )
        return self.executor.run(task)

    def _do_view_action(self, context, view, action, acls=[]):
        pool = self.get_pool(view.pool_id)
        tasks = zonetasks.ViewAction(
            self.executor, context, pool, view, action, acls)
        return self.executor.run(tasks)
    ...
    def create_zone(self, context, zone):
        ...
        :return: None
        """
        return self._do_zone_action(context, zone)
    ...
    def create_recordset(self, context, zone, recordset):
        return self._do_recordset_action(context, zone, recordset)
    ...
    def create_record(self, context, zone, record, recordset):
        return self._do_record_action(context, zone, record, recordset)
    ...
    def create_view(self, context, view, acls):
        action = 'CREATE'
        view = self._do_view_action(context, view, action, acls)
        return view
    
    def delete_view(self, context, view):
        action = 'DELETE'
        return self._do_view_action(context, view, action)
    
    def update_view(self, context, acls, view):
        action = 'UPDATE'
        return self._do_view_action(context, view, action, acls)

    def create_acl(self, context, acl):
        action = "CREATE"
        return self._do_acl_action(context, acl, action)
